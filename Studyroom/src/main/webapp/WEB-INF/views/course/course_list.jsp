...
<title>코스 목록</title>
	...
	<script type="text/javascript">		
	    if ("${insertSuccess***REMOVED***" == 1) {
	        alert('코스 추가 완료');
	    ***REMOVED***
	    if("${insertSuccess***REMOVED***" == 2){
	    	alert('실패했습니다')
	    ***REMOVED***
		...
		function delCheck(){
			if(xhr.readyState == 4 && xhr.status == 200){
				...
				if(json[0].result == 'yes'){
					alert('삭제 성공')
					location.href='course_list?id='+${user_dto.id***REMOVED***;
			***REMOVED*** else {
					alert('삭제 실패')
			***REMOVED***
		***REMOVED***
	       ...
	       <div class="titlediv text-center">
			    <h1 class="nanum-gothic-regular">코스 목록</h1>
   		        <c:if test="${sessionScope.dto.role eq 'admin'***REMOVED***">
              		<c:choose>
              		<c:when test="${role eq 'admin'***REMOVED***">
						<input id="insert_btn" type="button" class="btn btn-primary btn-sm" value="코스 만들기" onclick="insert()">
                    </c:when>
                    <c:otherwise>
                    	<input id="insert_btn" type="button" class="btn btn-primary btn-sm" value="코스 추가하기" onclick="user_course_insert('${user_dto.id***REMOVED***')">
                    </c:otherwise>
                    </c:choose>
            	</c:if>
			</div>
	               ...
	               <div class="col box col-4">
	               		<div class="gt item card d-flex">
	               			<span class="card-header fw-bold d-flex justify-content-between align-items-center">
		               			...
		               			<!-- choose when 으로 이 밑에 조건을 감싸는 test=${sessionScope.dto.role eq 'admin'***REMOVED*** 을 만들어서 버튼 바꾸기 -->
		               			<c:if test="${sessionScope.dto.role eq 'admin'***REMOVED***">
			               			<c:choose>
			               			<c:when test="${role eq 'admin'***REMOVED***">
				                    ...
				                    <c:otherwise>
				                    	<input id="manage-btn" type="button" class="btn btn-primary float-end" value="코스 제거" onclick="del('${user_dto.id***REMOVED***' , '${dto.id***REMOVED***')">
				                    </c:otherwise>
				                    </c:choose>
				            	</c:if>
		               		</span>
